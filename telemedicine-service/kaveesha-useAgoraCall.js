...
export function useAgoraCall({ sessionId, token, userToken }) {
  ...
  // Join channel
  const joinCall = useCallback(async ({ appId, channelName, agoraToken, uid }) => {
    try {
      setError(null);
      const client = clientRef.current;
      ...
      // Notify backend
      await fetch(`${API_BASE}/telemedicine/sessions/${sessionId}/join`, {
        method: 'POST',
  ...
  const leaveCall = useCallback(async () => {
    try {
      ...
      await fetch(`${API_BASE}/telemedicine/sessions/${sessionId}/leave`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${userToken}`, 'Content-Type': 'application/json' },
      });
    } catch (err) {
